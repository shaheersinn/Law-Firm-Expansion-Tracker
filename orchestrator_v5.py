...
BUG FIXES applied (v5.1):
  1. "SEC EDGAR Cross-Border" entry previously pointed to
     signals.deep.fiscal_calendar / CrossBorderIntelligence — that class
     does live there (it was added to fiscal_calendar.py as Part B), but
     the dedicated signals.cross_border_intel module contains the real,
     full implementation (SECEdgarCrossBorderTracker + LateralMagnetTracker).
     Both are now registered correctly.

  2. Four fully-implemented signal classes were orphaned — present in the
     codebase but never added to any schedule list, so they never ran:
       • SECEdgarCrossBorderTracker  (signals.cross_border_intel)
       • LateralMagnetTracker        (signals.cross_border_intel)
       • NewCourtFilingMonitor       (signals.deep.new_court_filings)
       • CompetitionBureauMonitor    (signals.deep.competition_bureau)
     All four are now wired in.
═══════════════════════════════════════════════════════════════════════
"""
...
BI_HOURLY = [
    ...
    # FIX: CrossBorderIntelligence is the fiscal_calendar.py internal helper
    # class (Part B of that file). The real dedicated cross-border module is
    # signals.cross_border_intel — replaced and split into two entries below.
    ...
    # FIX: newly registered — was implemented but never scheduled
...
def _run(strats):
    ...
    for item in strats:
        ...
        try:
            ...
            log.info("  ✅  %-40s %2d signals  %.1fs", name, len(sigs), time.time()-t0)
            total += len(sigs)
        except Exception as e:
            log.error("  ❌  %-40s FAILED: %s", name, e)
    return total
...
def run_full_pipeline_v5(mode: str = "bi-hourly"):
    ...
    total_strategies = len(BI_HOURLY) + len(DAILY) + len(WEEKLY) + len(SEASONAL)
    log.info("═" * 60)
    log.info("  CALGARY LAW TRACKER v5  [%s]  %d strategies registered",
             mode.upper(), total_strategies)
    log.info("═" * 60)
